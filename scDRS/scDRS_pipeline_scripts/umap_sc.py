...
os.makedirs("umap_hg38/S", exist_ok=True)
os.makedirs("umap_hg38/C", exist_ok=True)

print("Loading AnnData...")
adata = sc.read_h5ad("/biostack/home/suchenyi/scdrs/raw_hg.h5ad")

print("Discovering traits...")
score_files = sorted(glob.glob("score_hg38/*.full_score.gz")) 
...
for trait in loaded_traits:
    ...
    for subset, label in [(adata_S, "S"), (adata_C, "C")]:
        ...
        try:
            ...
            plt.savefig(
                f"umap_hg38/{label}/64human_{safe_name}_{label}.pdf",
                dpi=300,
                bbox_inches='tight'
            )
            plt.close()
